...
const useStyles = makeStyles((theme) => ({
  ...
  // Estilos para modais e dialogs permanecem os mesmos
  dialogContent: {
    position: 'relative',
    height: '65vh',
  ...
  // Estilos do PDF permanecem os mesmos
  header2: {
    textAlign: 'center',
  ...
  fabTutorial: {
    position: 'fixed',
    top: theme.spacing(3),          // antes era bottom
    right: theme.spacing(3),       // antes era left
    background: 'linear-gradient(135deg, #2d5a3d 0%, #4a7c59 100%)',
    color: '#fff',
    zIndex: 1000,
    '&:hover': {
...
const CompactSection: React.FC<{
  ...
  // Mock functions for drag and drop, as they are not defined in the original snippet
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      ...
      // Assuming onFileChange is intended to handle these dropped files
      // A more specific handler might be needed depending on the exact context
      const dummyEvent = { target: { files: droppedFiles } } as unknown as React.ChangeEvent<HTMLInputElement>;
      onFileChange(dummyEvent);
    }
  };
  ...
  return (
    ...
    >
      <Typography component="div" className={classes.sectionTitle}>
        <span style={{ flex: 1, fontFamily: '"Playfair Display", serif', fontWeight: 200, fontSize: '1.2rem' }}>
          {title}
        </span>
        <span style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          {title !== "Identificação do Comprador" && title !== "Solicitante" && ( // Condicional para ícones de câmera e lápis
      ...
      {processingImage && (
        <div className={classes.processingIndicator}>
          ...
          <span style={{ fontSize: '0.9rem' }}>Analisando documento...</span>
        </div>
      )}
...
const ListPost: React.FC<{ setItems: React.Dispatch<React.SetStateAction<Item[]>>, onItemEnviado?: () => void }> = ({ setItems, onItemEnviado }) => {
  ...
  // Estados dos serviços - agora permite apenas um
  ...
  // Funções helper (mantém a lógica original)
  ...
  // Função para processar imagens com IA (melhorada)
  const processImageWithAI = async (file: File, section: string) => {
    ...
    try {
      ...
      if (section === 'veiculo') {
        ...
        DADOS DO VEÍCULO: placa, renavam, chassi, modelo, marca, ano, cor, combustível, categoria, espécie, valor de venda ou IPVA.

        DADOS DO PROPRIETÁRIO/VENDEDOR: nome completo do proprietário, CPF do proprietário, endereço se houver.

        Formato JSON completo: {"placa": "", "renavam": "", "crv": "", "chassi": "", "modelo": "", "valordevenda": "", "ano": "", "cor": "", "marca": "", "nomeVendedor": "", "cpfVendedor": "", "enderecoVendedor": ""}`;
        ...
        prompt = `Analise este documento pessoal (RG/CPF/CNH) ou cartão CNPJ (Comprovante de Inscrição e Situação Cadastral) e extraia TODOS os dados em JSON. Se for pessoa física: nome, CPF, RG, data de nascimento, endereço se houver. Se for cartão CNPJ: razão social, nome fantasia, CNPJ, endereço, atividade principal, situação cadastral, data de abertura. Procure especificamente por termos como "COMPROVANTE DE INSCRIÇÃO", "SITUAÇÃO CADASTRAL", "RAZÃO SOCIAL", "NOME FANTASIA". Formato: {"nome": "", "cpf": "", "cnpj": "", "razaoSocial": "", "nomeFantasia": "", "endereco": "", "atividadePrincipal": "", "situacao": "", "dataAbertura": "", "rg": "", "dataNascimento": "", "categoria": "", "validade": ""}`;
      } else if (section === 'comprador_end') {
        prompt = `Analise este comprovante de endereço e extraia TODOS os dados de localização em JSON. Procure por: endereço completo, bairro, cidade, estado, CEP, nome do titular se houver. Formato: {"endereco": "", "bairro": "", "municipio": "", "cep": "", "estado": "", "nome": ""}`;
      } else if (section === 'comprador_residencia') {
        prompt = `Analise este comprovante de residência (conta de luz/água/telefone) e extraia TODOS os dados em JSON. Procure por: nome do titular, endereço completo, bairro, cidade, estado, CEP, tipo de conta. Formato: {"nome": "", "endereco": "", "bairro": "", "municipio": "", "cep": "", "estado": "", "tipoConta": ""}`;
      } else if (section === 'solicitante') {
        prompt = `Analise este documento (CPF, RG, CNH ou Cartão CNPJ) e extraia TODOS os dados em JSON. Se for pessoa física: nome, CPF, RG. Se for pessoa jurídica (Cartão CNPJ): razão social, nome fantasia, CNPJ, endereço, atividade principal, situação cadastral. Formato: {"nome": "", "cpf": "", "cnpj": "", "razaoSocial": "", "nomeFantasia": "", "endereco": "", "atividadePrincipal": "", "situacao": ""}`;
      }
  ...
  const extractDataManually = (text: string, section: string) => {
    ...
    try {
      if (section === 'solicitante') {
        ...
        // Procurar por CPF se não encontrar CNPJ
        if (!data.cnpj) {
  ...
  const extractCEPFromAddress = (enderecoCompleto: string) => {
    ...
    // Regex para extrair CEP no formato XX.XXX-XXX ou XXXXXXXX
  ...
  const fillFieldsFromAI = (data: any, section: string) => {
    ...
    setNewItem(prev => {
      ...
      if (section === 'veiculo') {
        ...
        // Dados do vendedor (extraídos do documento do veículo)
        ...
        // If it's a CNPJ document, update the company information
        if (data.cnpj) {
          ...
          // Extrair apenas CEP e usar ViaCEP para preencher endereço
          if (data.endereco) {
            const cep = extractCEPFromAddress(data.endereco);
            if (cep) {
          ...
          // Para documentos pessoais normais
          if (data.endereco) {
            const cep = extractCEPFromAddress(data.endereco);
            if (cep) {
              setTimeout(() => fetchFullAddressFromCEP(cep, 'comprador'), 500);
        ...
        // Se o nome do titular estiver no comprovante e não há nome do comprador ainda
        if (data.nome && !updated.nomecomprador) updated.nomecomprador = data.nome.toUpperCase();
      } else if (section === 'comprador_residencia') {
        // Para comprovante de residência, extrair CEP e usar ViaCEP
        if (data.endereco) {
          const cep = extractCEPFromAddress(data.endereco);
          if (cep) {
            setTimeout(() => fetchFullAddressFromCEP(cep, 'comprador'), 500);
        ...
        // Só preencher o nome se não existir ainda
        if (data.nome && !updated.nomecomprador) updated.nomecomprador = data.nome.toUpperCase();
      } else if (section === 'solicitante') {
        // Priorizar dados de pessoa jurídica se existirem
        if (data.cnpj) {
          updated.cnpjempresa = data.cnpj.replace(/\D/g, '');
          // Para CNPJ, usar razão social como nome principal
          if (data.razaoSocial) {
  ...
  const fetchAddressFromCEP = async (cep: string, fieldType: string) => {
    try {
      ...
      if (!data.erro) {
        setNewItem(prev => {
          const updated = { ...prev };
          if (fieldType === 'cepvendedor') {
            updated.enderecovendedor = data.logradouro || prev.enderecovendedor;
            updated.municipiovendedor = data.localidade || prev.municipiovendedor;
          } else if (fieldType === 'cepcomprador') {
            updated.enderecocomprador = data.logradouro || prev.enderecocomprador;
            updated.bairrocomprador = data.bairro || prev.bairrocomprador;
            updated.municipiocomprador = data.localidade || prev.municipiocomprador;
            updated.complementocomprador = data.uf || prev.complementocomprador;
          }
          return updated;
        });
      }
  ...
  const handleInputChange = (
    ...
    setNewItem(prev => {
      ...
      if ((field === 'cepvendedor' || field === 'cepcomprador')) {
        const cepLimpo = value.replace(/\D/g, '');
        if (cepLimpo.length === 8) {
          setTimeout(() => fetchAddressFromCEP(cepLimpo, field), 500);
        }
      }
  ...
  const uploadFiles = async (): Promise<string[]> => {
    ...
    setIsLoading(true);
    try {
      console.log('📤 Iniciando upload de arquivos públicos...');
      console.log(`📊 Total de arquivos para upload: ${toUpload.length}`);

      const uploadPromises = toUpload.map(async (file, index) => {
        const k = fileSig(file);
        const cached = uploadCacheRef.current.get(k);
        if (cached) {
          console.log(`✅ Arquivo ${index + 1} encontrado no cache`);
          return cached;
        }
        ...
        console.log(`📤 Uploading arquivo ${index + 1}/${toUpload.length}: ${fileName}`);
        ...
        try {
          ...
          console.log(`✅ Arquivo ${index + 1} enviado com sucesso: ${url}`);
          return url;
        } catch (uploadError: any) {
          console.error(`❌ Erro ao fazer upload do arquivo ${index + 1}:`, {
            fileName,
      ...
      console.log(`✅ Todos os ${urls.length} arquivos foram enviados com sucesso`);
      return urls;
    } catch (error: any) {
      console.error('❌ Erro geral no upload de arquivos:', {
  ...
  const handleAddItem = async () => {
    ...
    try {
      ...
      // Salvar no Firestore principal
      console.log('📝 Tentando salvar documento em Betodespachanteintrncaodevendaoficialdigital...');
      console.log('📋 Dados:', { ...itemParaSalvar, docUuid });
      ...
      if (error && typeof error === 'object' && 'code' in error) {
        ...
        errorMessage = `Erro do Firebase (${firebaseError.code}): ${firebaseError.message}`;

        if (firebaseError.code === 'permission-denied') {
          errorMessage = '⚠️ ATENÇÃO: Erro de permissão ao salvar!\n\n' +
            'As regras de segurança do Firebase precisam ser atualizadas:\n\n' +
            '📋 PASSO A PASSO:\n' +
            '1. Acesse: https://console.firebase.google.com\n' +
            '2. Selecione o projeto "betoappteste"\n' +
            '3. Vá em "Firestore Database" → "Regras"\n' +
            '4. Copie o conteúdo do arquivo "regrasfirebase.md"\n' +
            '5. Cole e clique em "Publicar"\n' +
            '6. Vá em "Storage" → "Regras"\n' +
            '7. Copie o conteúdo do arquivo "regrasstorege.md"\n' +
            '8. Cole e clique em "Publicar"\n\n' +
            '⏱️ Aguarde 1-2 minutos e tente novamente.\n\n' +
            '🔍 Erro técnico: ' + firebaseError.message;
        } else if (firebaseError.code === 'storage/unauthorized') {
          errorMessage = '⚠️ Erro ao fazer upload dos arquivos!\n\n' +
            'As regras do Firebase Storage precisam permitir acesso público.\n\n' +
            'Verifique o arquivo "regrasstorege.md" e publique as regras no Firebase Console.';
        }
      } else if (error instanceof Error) {
        errorMessage = error.message;
      }

      alert(`❌ Ocorreu um erro ao salvar o requerimento:\n\n${errorMessage}`);
    } finally {
      setIsLoading(false);
      setIsSubmitting(false);
      savingRef.current = false;
    ...
    <div className={classes.root}>
      ...
      <Paper className={classes.formContainer}>
        ...
        <div className={classes.compactSection}>
          <Typography component="div" className={classes.sectionTitle}>Selecione os Serviços</Typography>
          <div className={classes.servicesContainer}>
             {['ATPV - ASSINATURA - COMUNICAÇÃO DE VENDA', 'ATPV - COMUNICAÇÃO DE VENDA', 'ATPV - ASSINATURA'].map((produto) => (
              <Button
                key={produto}
          ...
          <Grid item xs={12} md={6}>
            <div
              ...
              onDrop={(e) => {
                ...
                if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
                  const droppedFiles = Array.from(e.dataTransfer.files) as File[];
                  setVeiculoFiles(prev => [...prev, ...droppedFiles]);

                  if (droppedFiles.length > 0) {
                    processImageWithAI(droppedFiles[0], 'veiculo');
                  }
                }
              }}
              style={{ transition: 'all 0.2s ease' }}
              ...
              <Typography component="div" className={classes.sectionTitle}>
                ...
                <span style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                  ...
                  <Tooltip title="Preencher Manual">
                    <IconButton
                      size="small"
                      onClick={() => setVeiculoExpanded(!veiculoExpanded)}
                      style={{
                        backgroundColor: 'rgba(45, 90, 61, 0.1)',
                        color: '#2d5a3d',
                        width: '32px',
                ...
                <Grid container spacing={2} alignItems="center">
                  <Grid item xs={12} sm={files.length > 0 ? 6 : 12}>
                    <input
                      accept="image/*,application/pdf"
                      style={{ display: 'none' }}
                      id={`upload-${'Identificação do Veículo'.replace(/\s+/g, '-')}`}
                      type="file"
                      onChange={handleVeiculoFileChange}
                      multiple
                    />
                    <label htmlFor={`upload-${'Identificação do Veículo'.replace(/\s+/g, '-')}`}>
                      <Button
                        variant="contained"
                        component="span"
                        className={classes.uploadButton}
                  ...
                  {veiculoFiles.length > 0 && (
                    <Grid item xs={12} sm={6}>
                      <div className={classes.thumbnailContainer}>
                        {veiculoFiles.map((file, index) => (
                          <div key={index} className={classes.thumbnail}>
                            <img src={URL.createObjectURL(file)} alt={`${'Identificação do Veículo'} ${index}`} />
                            <IconButton
                              className={classes.deleteButton}
                              size="small"
                              onClick={() => setVeiculoFiles(prev => prev.filter((_, i) => i !== index))}
                            >
                              <Delete fontSize="small" />
                            </IconButton>
                          </div>
          ...
          <Grid item xs={12} md={6}>
            <div
              ...
              onDrop={(e) => {
                ...
                if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
                  const droppedFiles = Array.from(e.dataTransfer.files) as File[];
                  setVendedorFiles(prev => [...prev, ...droppedFiles]);

                  if (droppedFiles.length > 0) {
                    processImageWithAI(droppedFiles[0], 'vendedor');
                  }
                }
              }}
              style={{ transition: 'all 0.2s ease' }}
              ...
              <Typography component="div" className={classes.sectionTitle}>
                ...
                <span style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                  ...
                  <Tooltip title="Preencher Manual">
                    <IconButton
                      size="small"
                      onClick={() => setVendedorExpanded(!vendedorExpanded)}
                      style={{
                        backgroundColor: 'rgba(45, 90, 61, 0.1)',
                        color: '#2d5a3d',
                        width: '32px',
                ...
                <Grid container spacing={2} alignItems="center">
                  <Grid item xs={12} sm={files.length > 0 ? 6 : 12}>
                    <input
                      accept="image/*,application/pdf"
                      style={{ display: 'none' }}
                      id={`upload-${'Identificação do Vendedor'.replace(/\s+/g, '-')}`}
                      type="file"
                      onChange={handleVendedorFileChange}
                      multiple
                    />
                    <label htmlFor={`upload-${'Identificação do Vendedor'.replace(/\s+/g, '-')}`}>
                      <Button
                        variant="contained"
                        component="span"
                        className={classes.uploadButton}
                  ...
                  {vendedorFiles.length > 0 && (
                    <Grid item xs={12} sm={6}>
                      <div className={classes.thumbnailContainer}>
                        {vendedorFiles.map((file, index) => (
                          <div key={index} className={classes.thumbnail}>
                            <img src={URL.createObjectURL(file)} alt={`${'Identificação do Vendedor'} ${index}`} />
                            <IconButton
                              className={classes.deleteButton}
                              size="small"
                              onClick={() => setVendedorFiles(prev => prev.filter((_, i) => i !== index))}
                            >
                              <Delete fontSize="small" />
                            </IconButton>
                          </div>
        ...
        <div className={classes.compactSection}>
          ...
          <Collapse in={documentosExpanded}>
            <div className={classes.expandableContent}>
              <div style={{ marginBottom: 16, color: '#666', fontSize: '0.875rem' }}>
      ...
      >
        <DialogContent>
          ...
          <Typography variant="caption" style={{ textAlign: 'center', display: 'block', color: '#666' }}>
