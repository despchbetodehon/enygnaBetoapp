...
// ===== Helpers para deduplicação e cache de upload =====
...
const ListPost: React.FC<{ setItems: React.Dispatch<React.SetStateAction<Item[]>>, onItemEnviado?: () => void }> = ({ setItems, onItemEnviado }) => {
  ...
  const [isLoading, setIsLoading] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false); // trava UI no submit
  const savingRef = useRef(false); // trava dura anti duplo clique
  const uploadCacheRef = useRef(new Map<string, string>()); // cache de uploads (assinatura de arquivo -> URL)
    ...
    if (videoRef.current && canvasRef.current) {
      ...
      canvasRef.current.toBlob((blob) => {
        if (blob) {
          ...
          setFiles(prev => uniqueFiles([...prev, file]));
          setCameraOpen(false);
        }
      }, 'image/jpeg', 0.9);
    }
  ...
  const cropImage = async () => {
    try {
      ...
      setFiles(prev => uniqueFiles(prev.map((f, i) => i === currentImageIndex ? file : f)));
      setCropOpen(false);
    } catch (e) {
  ...
  const uploadFiles = async (): Promise<string[]> => {
    ...
    try {
      const uploadPromises = toUpload.map(async (file) => {
        ...
        const storageRef = ref(storage, `uploads/${k}-${Date.now()}`);
  ...
  const handleAddItem = async () => {
    if (savingRef.current) return; // trava dura contra cliques repetidos
    ...
    try {
      ...
      // 🔑 AGORA O NOME DO DOCUMENTO NO FIRESTORE É UM UUID (mantemos o comportamento original)
      ...
      await setDoc(
        ...
        {
          ...itemParaSalvar,
          docUuid, // guarda o uuid também dentro do doc para referência
        },
        { merge: true }
      );
    ...
    <div className={`${classes.formContainer} ${classes.noPrint}`}>
      <Paper className={classes.formContainer}>
        ...
        <div className={classes.actionBar}>
          ...
          >
            {isLoading || isSubmitting ? <CircularProgress size={24} /> : 'Enviar Requerimento'}
          </Button>
        </div>
      </Paper>
